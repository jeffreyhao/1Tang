...
public class ProgressWheel extends View {
  ...
  private void setupBounds(int layout_width, int layout_height) {
    ...
    if (!fillRadius) {
      // Width should equal to Height, find the min value to setup the seekbar_style
  ...
  /**
   * @return the base spinning speed, in full seekbar_style turns per second
  ...
  /**
   * Sets the base spinning speed, in full seekbar_style turns per second
   * (1.0 equals on full turn in one second), this value also is applied for
   * the smoothness when setting a progress
   *
